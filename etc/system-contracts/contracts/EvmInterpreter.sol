...
contract EvmInterpreter {
    ...
    function _simulate(bytes memory code, bytes calldata input, uint256 gasLeft) internal {
        unchecked {
            ...
            // Note: Workaround for static calls for demonet. Allocate a large block for return data but set its size to 0 to not
            bytes memory returnData = new bytes(8192);
            assembly ("memory-safe") {
                mstore(returnData, 0)
            }
            ...
            while (true) {
                ...
                if (opcode < GRP_VM_STATE) {
                    ...
                    if (opcode == OP_RETURNDATACOPY) {
                        ...
                        EVM_GAS_MANAGER.returnDataCopy(returnData);
                ...
                if (opcode == OP_CALL || opcode == OP_DELEGATECALL || opcode == OP_STATICCALL) {
                    ...
                    uint256 frameGasUsed = tmp - EVM_GAS_MANAGER.popGasLeft();
                    if (frameGasUsed == 0) {
